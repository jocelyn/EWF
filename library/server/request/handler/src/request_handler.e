...
feature -- Execution
	...
	response (a_path: STRING; req: GW_REQUEST): GW_RESPONSE
		...
		do
			if not rescued then
				...
				if request_method_name_supported (req.environment.request_method) then
					pre_execute (req)
					Result := application_response (a_path, req, l_format, l_args)
					post_execute (req)
				else
					Result := response_method_not_allowed (req, l_format, l_args)
				end
			else
...
--		end
	...
	response_method_not_allowed (req: GW_REQUEST; a_format: detachable STRING; a_args: detachable STRING): GW_RESPONSE
		local
			h: GW_HEADER
			s: STRING
			lst: LIST [STRING]
	...
	post_execute (req: GW_REQUEST)
			-- Operation processed after `execute'
		do
			--| To be redefined if needed
		end

	rescue_execute (req: GW_REQUEST)
