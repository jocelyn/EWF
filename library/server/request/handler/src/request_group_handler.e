...
feature -- Execution
	...
	application_response (a_path: STRING; req: GW_REQUEST; a_format: detachable STRING_8; a_args: detachable STRING_8): GW_RESPONSE
		...
		do
			...
			if rq /= Void then
				...
				Result := rq.response (l_path, req)
			else
				Result := response_missing_application (req, a_format, a_args)
			end
		end

	response_missing_application (req: GW_REQUEST; a_format: detachable STRING_8; a_args: detachable STRING_8): GW_RESPONSE
		do
			Result := req.matching_response
			Result.write_header ({HTTP_STATUS_CODE}.not_acceptable, Void)
		end
