note
	description : "chatroom application execution"
	date        : "$Date$"
	revision    : "$Revision$"

class
	CHATROOM_APPLICATION_EXECUTION
		...
		local
 		...
 		do
			...
			if request.path_info.same_string_general ("/favicon.ico") then
				...
				if request.path_info.same_string_general ("/room") then
					...
					s.append ("<p><a href=%"/room%">Chatroom Websocket demo</a></p>")
				end
...
feature -- Command
	...
	initialize_commands
		do
			...
			register_command (agent on_message_command, "message", "Send private message, usage: /message @username1 {@username2} the message...")
			...
			register_command (agent on_loop_command, "loop", "start/stop looping echo (for testing).")
	...
	on_message_command (ws: WEB_SOCKET; args: detachable READABLE_STRING_GENERAL)
		local
			s: STRING_32
			msg: STRING_32
		...
		do
			if args /= Void then
				create s.make_from_string_general (args)
				s.left_adjust
				from
					create lst.make (1)
				until
					msg /= Void or s.is_whitespace
				loop
					if s[1] = '@' then
						from
							i := 1
						...
						if i <= n and then s[i].is_space then
							lst.force (s.substring (2, i - 1))
							s.remove_head (i)
						else
							lst.force (s.substring (2, n))
				...
				if msg /= Void and then not msg.is_whitespace and then not lst.is_empty then
					separate chat_room as r do
						r.send_message_to (msg, lst)
					end
				end
			end
		end
...
$(document).ready(function() {
	...
	function connect(username){
			var host = "##WSSCHEME##://##HOSTNAME##:##PORTNUMBER##/room/user=" + username;
			try{
...
<title>Room (WebSocket) Client</title>
</head>
<body>
  <div id="wrapper">
  	<div id="container">
    	<h1>Room (WebSocket) Client</h1>
